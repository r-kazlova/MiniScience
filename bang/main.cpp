...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = 10* sin(pow(pointY*pointY * pointZ, 2)*M_PI/180);
            //Здесь я пыталась взорвать, но получается взрыв треугольничками, возможно, стоит добавить шумов,
            //но это я не знаю как сделать
            double vx = (rand() % 50);
            double vy = (rand() % 40);
            double vz = (rand() % 30);
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "C:\\Users\\Risha\\CLionProjects\\MiniScience\\bang\\vec" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    // Считаем STL
    try {
        gmsh::merge("C:\\Users\\Risha\\CLionProjects\\MiniScience\\bang\\Skrepka.stl");
    } catch(...) {
